...
func (p *postgresDB) CreateUser(user *models.Users) (*models.Users, error) {
	queryStr := fmt.Sprintf("INSERT INTO %s (id, username, password, email, card_id, santri_id, created_at, updated_at)", usersTable)
	queryStr += "VALUES($1, $2, $3, $4, $5, $6, $7, $8)"

	_, err := p.db.Exec(queryStr, user.ID, user.Username, user.Password, user.Email, user.CardID, user.SantriID, time.Now(), time.Now())
	if err != nil {
